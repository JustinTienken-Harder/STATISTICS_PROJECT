...
We fail to reject the null hypothesis, with the confidence interval in agreement with our p-value. Our effect size is also practically insignificant.  
```{r echo = FALSE}
two.test.price
price.cohens
```
...
Due to our p-value and our confidence interval not included 0, we reject our null hypothesis.  While our effect size is small, it is not negligible.  
```{r echo = FALSE}
two.test.score
score.cohens
```
...
- Our first look into this was to create a probability tree of the intersections of the probabilities of a hostel being close or far from a city center, and a user rating the hostel are very worth the money and not worth the money. 
...
- Because so few users rate hostel visits poorly, it is hard to see an impact of distance on the probability of finding a stay to be worth the price.  
- However, in the conditional probabilities that a slightly larger, while negligibly so, probability of a user rating the hostel low value given that it was farther from a city center.  
- Results would be clearer if we conducted the same analysis for individual cities, because each city will bring a different meaning to far or close to the city center, and thus the impact of distance on value may be different.  
